...
void BluetoothConnect::onRemoteControl(BluetoothControlRemoteControlCallback callback) {
...
void BluetoothConnect::StateChangeCharacteristicsCallbacks::onWrite(BLECharacteristic *characteristic) {
    std::string bytes = characteristic->getValue();
    if (bytes.length() == sizeof(StateChangeCommand)) {
        if (bluetoothConnect->remoteControlCallback != nullptr) {
            ...
            if (CHECK_BIT(stateChangeCommand.mode, STATE_TRANSITION_MODE_BIT_COLOR)) {
                ...
                bluetoothConnect->floower->transitionColor(color.H, color.S, color.B, stateChangeCommand.duration * 100);
                bluetoothConnect->remoteControlCallback();
            }
            if (CHECK_BIT(stateChangeCommand.mode, STATE_TRANSITION_MODE_BIT_PETALS)) {
                // petals open/close
                bluetoothConnect->floower->setPetalsOpenLevel(stateChangeCommand.value, stateChangeCommand.duration * 100);
                bluetoothConnect->remoteControlCallback();
            }
                ...
                switch (stateChangeCommand.value) {
                    case 1:
                        bluetoothConnect->floower->startAnimation(FloowerColorAnimation::RAINBOW_LOOP);
                        bluetoothConnect->remoteControlCallback();
                        break;
                    case 2:
                        bluetoothConnect->floower->startAnimation(FloowerColorAnimation::CANDLE);
                        bluetoothConnect->remoteControlCallback();
                        break;
                }
            }
...
void BluetoothConnect::ColorsSchemeCharacteristicsCallbacks::onWrite(BLECharacteristic *characteristic) {
    std::string bytes = characteristic->getValue();
    if (bytes.length() > 0) {
        ...
        bluetoothConnect->config->setColorScheme(colors, size);
        bluetoothConnect->config->commit();
    }
}

void BluetoothConnect::NameCharacteristicsCallbacks::onWrite(BLECharacteristic *characteristic) {
    std::string bytes = characteristic->getValue();
    if (bytes.length() > 0) {
        ...
        bluetoothConnect->config->setName(String(data));
        bluetoothConnect->config->commit();
    }
}

void BluetoothConnect::PersonificationCharacteristicsCallbacks::onWrite(BLECharacteristic *characteristic) {
    std::string bytes = characteristic->getValue();
    if (bytes.length() == sizeof(Personification)) {
        ...
        bluetoothConnect->config->setPersonification(personification);
        bluetoothConnect->config->commit();
        //bluetoothConnect->floower->reconfigureTouch();
    }
}
